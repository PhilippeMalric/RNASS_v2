...
class RDATFile(object):
    def __init__(self):
        ...
        self.warningPos = defaultdict(list)
        self.mutpos = defaultdict(list)
        self.normalNt = defaultdict(list)
        self.mutNt = defaultdict(list)
        self.seqMuted = defaultdict(list)
    ...
    def _annotation_str_toFile(self, a, firstC,folder):
        for k in a:
            f = open(folder+k+".csv",'a')
            if isinstance(a[k], list):
                for i in range(len(a[k])):
                    f.write(firstC+","+a[k][i]+"\n")
            else:
                f.write(firstC+","+a[k]+"\n")
            ...
            if self.version == 0.1:
                ...
                if line.startswith('COMMENT'):
                    ...
                    if 'warning' in annotations:
                        self.warningPos[this_construct].append(1)
                    else:
                        self.warningPos[this_construct].append(0)
                    for l in range(data_idx - len(self.constructs[this_construct].data) + 1):
                        self._append_new_data_section(this_construct)
                    self.constructs[this_construct].data[data_idx].annotations = annotations
                    if 'mutation' in annotations:
                        try:
                            #print("mutation : "+annotations['mutation'][0])
                            if (annotations['mutation'][0] == 'WT'):
                                #print()
                                self.mutpos[this_construct].append('-')
                                self.normalNt[this_construct].append(str('-'))
                                self.mutNt[this_construct].append(str('-'))
                                self.seqMuted[this_construct].append(self.constructs[this_construct].sequence)
                            else:
                                tabTempMutpos = []
                                tabTempNormalNt = []
                                tabTempMutNt = []
                                tabseqMuted = []
                                for i in range (0, len(annotations['mutation'])):
                                    tabTempMutpos.append(int(annotations['mutation'][i][1:-1]))
                                    tabTempNormalNt.append(str(annotations['mutation'][i][0]))
                                    tabTempMutNt.append(str(annotations['mutation'][i][-1]))
                                    if hasattr(self.constructs[this_construct],"offset"):
                                        self.seqMuted[this_construct].append(self.mutate (self.constructs[this_construct].sequence, tabTempMutpos[-1] - self.constructs[this_construct].offset ,tabTempMutNt[-1]))
                                self.mutpos[this_construct].append(tabTempMutpos)
                                self.normalNt[this_construct].append(tabTempNormalNt)
                                self.mutNt[this_construct].append(tabTempMutNt)
                                
                        except ValueError:
                            pass
                #voir fin document pour du code enlevez
    ...
    def outStr(self, version=None, delim='\t'):
        outD = {}
        if not version:
        ...
        if not self.loaded:
            ...
            if version == 0.1:
                ...
                if len(self.constructs) == 1:
                    ...
                    #------------------------------------------------------------------------------
                    if self.comments:
                        outD[name].comments = self.comments
                        for com in self.comments.split('\n'):
                            f.write('COMMENT%s%s\n' % (delim, com))
                        f.write('\n')
...
if len(self.mutpos[this_construct]) > 0:
	self.mutpos[this_construct][-1] = int(annotations['mutation'][0][1:-1])
	self.normalNt[this_construct] = str(annotations['mutation'][0][0])
	self.mutNt[this_construct] = str(annotations['mutation'][0][-1])
else:
'''