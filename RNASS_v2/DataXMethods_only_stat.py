...
id_unique = (keyWord + "_Filter_" + filesFilter + pred_str + "_A_" + str(adenineCutOff) + "_S_" + str(
    score_seuil) + "_stn_" + str(stn_seuil) + "_ed_" + str(ed_seuil)).replace(".", "_").replace("-", "_")
                ...
                if (hasattr(rdat.constructs[key].data[0], "annotations")):
                    ...
                    if ("sequence" in rdat.constructs[key].data[0].annotations):
                        ...
                        for i in range(0, len(rdat.seqTab)):
                            seq = rdat.seqTab[i]
                            (ed, freqMfe) = rnaFold(dir_of_prog + "fasta_for_RNA_fold/",
                                                    root + "_" + str(i) + id_unique, seq)
                            ...
                            c1 = ("modifier" in rdat.constructs[key].data[0].annotations and
                                  rdat.constructs[key].data[i].annotations["modifier"][0] == "1M7")
...
if (not prediction):
    ...
    collection = "test_1__Filter__All_w_up__noPred__A_0_5_S_1_0_stn_1_0_ed_2_0"
    if (False):
                    ...
                    for nt in t:
                        ...
                        if (nt["score"] < low_threshold and nt["score"] != -999):
                            ...
                            db[collection].insert(
                                {"ncm": root, "soft": dPath["soft"], "url": url, "freq": freq, "score": score,
                                 "so_pairee": so_pairee, "mcff_pairee": mcff_pairee, "label": label})
    ...
    collection_out = "test_1__Filter__All_w_up__noPred__A_0_5_S_1_0_stn_1_0_ed_2_0_stat"
